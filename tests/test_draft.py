...
class TestCalculateInflation:
    """Tests for calculate_inflation."""

    def test_no_picks_returns_zeros(self, session, populated_db, test_settings):
        """Test that empty draft returns zero inflation."""
        initialize_draft(session, test_settings, "My Team")

        inflation = calculate_inflation(session)

        assert inflation['inflation_rate'] == 0.0
        assert inflation['num_picks'] == 0
        assert inflation['total_spent'] == 0
        assert inflation['overpays'] == 0
        assert inflation['bargains'] == 0

    def test_returns_all_expected_keys(self, session, populated_db, test_settings):
        """Test that function returns all expected keys."""
        initialize_draft(session, test_settings, "My Team")
        teams = get_all_teams(session)

        # Draft a player (uses fixture values: Trout=$50)
        draft_player(session, populated_db[0].id, teams[0].id, 45)

        inflation = calculate_inflation(session)

        expected_keys = [
            'inflation_rate', 'total_spent', 'total_projected_value',
            'num_picks', 'overpays', 'bargains', 'avg_difference',
            'by_type', 'recent_trend'
        ]
        for key in expected_keys:
            assert key in inflation

    def test_tracks_total_spent(self, session, populated_db, test_settings):
        """Test that total_spent is correctly summed."""
        initialize_draft(session, test_settings, "My Team")
        teams = get_all_teams(session)

        draft_player(session, populated_db[0].id, teams[0].id, 45)
        draft_player(session, populated_db[1].id, teams[1].id, 35)

        inflation = calculate_inflation(session)

        assert inflation['total_spent'] == 80
        assert inflation['num_picks'] == 2

    def test_counts_overpays_and_bargains(self, session, populated_db, test_settings):
        """Test that overpays and bargains are counted correctly."""
        initialize_draft(session, test_settings, "My Team")
        teams = get_all_teams(session)

        # Draft two players
        draft_player(session, populated_db[0].id, teams[0].id, 60)
        draft_player(session, populated_db[1].id, teams[1].id, 30)

        inflation = calculate_inflation(session)

        # Every pick is either an overpay, bargain, or exact match
        # (overpays + bargains) <= num_picks
        assert inflation['overpays'] + inflation['bargains'] <= inflation['num_picks']
        assert inflation['overpays'] >= 0
        assert inflation['bargains'] >= 0

    def test_by_type_separates_hitters_and_pitchers(self, session, populated_db, test_settings):
        """Test that by_type separates hitters and pitchers."""
        initialize_draft(session, test_settings, "My Team")
        teams = get_all_teams(session)

        # Draft a hitter (index 0) and a pitcher (index 3)
        draft_player(session, populated_db[0].id, teams[0].id, 45)  # hitter
        draft_player(session, populated_db[3].id, teams[1].id, 30)  # pitcher

        inflation = calculate_inflation(session)

        assert 'hitter' in inflation['by_type']
        assert 'pitcher' in inflation['by_type']
        assert inflation['by_type']['hitter']['num_picks'] == 1
        assert inflation['by_type']['pitcher']['num_picks'] == 1

    def test_recent_trend_calculated(self, session, populated_db, test_settings):
        """Test that recent_trend is calculated when enough picks exist."""
        initialize_draft(session, test_settings, "My Team")
        teams = get_all_teams(session)

        # Draft multiple players to have enough data for trend
        draft_player(session, populated_db[0].id, teams[0].id, 45)
        draft_player(session, populated_db[1].id, teams[1].id, 35)
        draft_player(session, populated_db[2].id, teams[2].id, 40)
        draft_player(session, populated_db[3].id, teams[3].id, 30)
        draft_player(session, populated_db[4].id, teams[0].id, 28)

        inflation = calculate_inflation(session)

        # recent_trend should be a float (could be positive, negative, or zero)
        assert isinstance(inflation['recent_trend'], float)
